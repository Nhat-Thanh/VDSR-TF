...
model_path = os.path.join(ckpt_dir, f"VDSR-x{scale}.h5")
...
dataset_dir = "dataset"
lr_crop_size = 33
hr_crop_size = lr_crop_size 

train_set = dataset(dataset_dir, "train")
train_set.generate(lr_crop_size, hr_crop_size, scale)
train_set.load_data()

valid_set = dataset(dataset_dir, "validation")
valid_set.generate(lr_crop_size, hr_crop_size, scale)
valid_set.load_data()

test_set = dataset(dataset_dir, "test")
test_set.generate(lr_crop_size, hr_crop_size, scale)
test_set.load_data()
...
vdsr.load_weights(model_path)
loss, metric = vdsr.evaluate(test_set)
